...
async function getResumenVenta(req, res) {
  try {
    ...
    const result = await request.execute('[usp_GetResumenVentas]'); // llamar al procedimiento almacenado

    res.json({ success: true, data: result.recordset });
  } catch (err) {
    console.error('Error al obtener Resumen:', err);
    res.status(500).json({ success: false, message: 'Error al obtener información de resumen díario' });
  }
}

module.exports = { getResumenVenta };