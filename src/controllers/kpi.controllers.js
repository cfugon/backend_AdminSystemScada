...
async function getDashboard(req, res) {
  try {
    ...
    const result = await request.execute('usp_GetKPI'); // llamar al procedimiento almacenado

    console.log('resultado Dashboard KPI', result.recordset);

    res.json({ success: true, data: result.recordset });
  } catch (err) {
    console.error('Error al obtener Kardex:', err);
    res.status(500).json({ success: false, message: 'Error al obtener Dashbaord' });
  }
}

//sin procedimiento almacenado
async function getDashboardTest(req, res) {
  try {
    const pool = await getPool();
